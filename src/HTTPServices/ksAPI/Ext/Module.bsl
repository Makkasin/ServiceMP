...
Функция GETGET(Запрос)
	...
	ИначеЕсли Метод = "RECOGNIZETS" Тогда
		Результат = РаспознатьНомер(Запрос.ПолучитьТелоКакСтроку());
	ИначеЕсли Метод = "RECOGNIZETSMAIL" Тогда
		Результат = РаспознатьНомерМэйл(Запрос.ПолучитьТелоКакСтроку());
...
Функция РаспознатьНомерМэйл(пТело)
...
Функция РаспознатьНомер(пТело)
	мДвоичныеДанные = XMLзначение(Тип("ХранилищеЗначения"), пТело).Получить();
	мСтрокаНомера = ЗапросРаспознаванияВРекор(мДвоичныеДанные);
	вСтруктура = НайтиТСПоНомеру(мСтрокаНомера);
	вХранилище = Новый ХранилищеЗначения(вСтруктура, Новый СжатиеДанных(5));
	Возврат XMLСтрока(вХранилище);
КонецФункции
...
Функция ЗапросРаспознаванияВРекор(пДвоичныеДанные)
	Соединение = Новый HTTPСоединение(
	"api.openalpr.com", // сервер (хост)
	443, // порт, по умолчанию для http используется 80, для https 443
	, // пользователь для доступа к серверу (если он есть)
	, // пароль для доступа к серверу (если он есть)
	, // здесь указывается прокси, если он есть
	...
	Запрос = Новый HTTPЗапрос("/v3/recognize_bytes?secret_key=sk_37faef419dd451e46e51a1be&country=eu");
	Запрос.Заголовки.Вставить("secret_key","sk_37faef419dd451e46e51a1be");
	Запрос.Заголовки.Вставить("country","eu");
	
	Стр = Base64Строка(пДвоичныеДанные);
	...
	//мДанныеОтвета.получить("error");
	//мДанныеОтвета.получить("results")[0].получить("vehicle_detected");
	//Сообщить("1 символ = А ? " + ("А" = Сред(мДанныеОтвета.получить("results")[0].Получить("plate"), 1, 1)));
	Если мДанныеОтвета.получить("results").Количество() > 0 Тогда
		Возврат мДанныеОтвета.получить("results")[0].Получить("plate");
	Иначе
		Возврат "НЕРАСПОЗНАНО";
	КонецЕсли;
КонецФункции