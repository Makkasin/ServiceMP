...
Функция ДанныеОРГ(масИНН=Неопределено) Экспорт
	...
	Если масИНН<>Неопределено Тогда
		
		флтИНН = "''";
		ДЛя каждого эл из МасИНН Цикл
			флтИНН = флтИНН +", '"+СокрЛП(эл)+"'";
		КонецЦикла;
		
		ТекстЗапроса = ТекстЗапроса + "
		|  WHERE Code IN ("+флтИНН+") ";
...
Функция фрмТолькоЧисла(Стр) Экспорт
...
Процедура ЗагрузитьProduct() Экспорт
	...
	//Данныеproduct("8603104188");  //Юрал актив
...
Функция ОтправитьПисьмоОПредстоящихДняхРождения(Тест=Ложь,ДнейДоДняРождения=0,УстановитьОграничениеОтправки=Истина) Экспорт
	...
	ПолучателиДляВсехБаз.Добавить("antonenkodv@urals.pro");
	...
	Если Не Тест Тогда
		...
		ПолучателиДляТекущейБазы.Добавить("suslovans@mtk-1.ru");
		ПолучателиДляТекущейБазы.Добавить("galayan@mtk-1.ru");
		ПолучателиДляТекущейБазы.Добавить("kornevaon@mtk-1.ru");
	КонецЕсли;	
	
	ТЗДанные=ПолучитьДанныеСотрудников("8906006670","188.17.143.98",47947,"HRM31","ksAPI");
...
Функция ПолучитьОстаткиЕдСкладПоНом(Тело) Экспорт
	...
	Для каждого эл из Мас Цикл
		СтрПск = СтрПск+",'"+СокрЛП(Эл)+"'"
	КонецЦикла;
	СтрПск = Сред(СтрПск,2);
	...
	Возврат ЗаписатьХранилищеВСтроку(Таблица);
	
	
КонецФункции

Функция ПолучитьОстаткиЕдСкладПоНомИРегионамЛокации(Тело) Экспорт
	
	Структура = XMLзначение(Тип("ХранилищеЗначения"),тело).Получить();
	
	КодНоменклатур = Неопределено;
	РегионыЛокации = Неопределено;
	ИННОрганизации = Неопределено;
	
	Структура.Свойство("КодНоменклатур", КодНоменклатур);
	Структура.Свойство("РегионыЛокации", РегионыЛокации);
	Структура.Свойство("ИННОрганизации", ИННОрганизации);
	
	Если КодНоменклатур = Неопределено
	 Или РегионыЛокации = Неопределено
	 Или ИННОрганизации = Неопределено Тогда 
		Возврат Неопределено;
	КонецЕсли;
	
	РегионыЛокации = СтрРазделить(РегионыЛокации, " ,;", Ложь);
	РегионыЛокации = СтрСоединить(РегионыЛокации, "','");
	
	Соединение = глОбщий.ПолучитьСоединениеAZURESQL();	
	
	// productBalance21: innOrg, nameOrg, idNom, Nom, code, ed, dt, warehouse, yr, Kol, price, Sum, dtSynchro, indKol0
	// 				org: orgId, Code, descr, extIp, intIP, dtSynchro, apiRest, apiRestEnt, codeRegion, extIpENT, IntIpENT, ShortName
	
	НаборЗаписей = Новый COMObject("ADODB.Recordset");
	НаборЗаписей.Open(
	"SELECT
	|	org.codeRegion as КодРегиона,
	|	org.ShortName as НаименованиеСокращенное,
	|	ost.innOrg as ИНН,
	|	ost.nameOrg as Организация,
	|	ost.warehouse as Склад,
	|	ost.idNom as НоменклатураСсылка,
	|	ost.code as Код,
	|	ost.Nom as Наименование,
	|	ost.Kol as Количество,
	|	ost.price as Цена,
	|	ost.dt as Дата
	|FROM
	|	productBalance21 as ost
	|	INNER JOIN org
	|		ON ost.innOrg = org.Code
	|			AND org.codeRegion IN (SELECT codeRegion FROM org WHERE codeRegion = '"+РегионыЛокации+"' OR Code = '"+ИННОрганизации+"')
	|WHERE
	|	ost.Code IN ('"+СтрСоединить(КодНоменклатур, "','")+"')
	|ORDER BY
	|	Nom"
	, Соединение);
	...
	Таблица = Новый ТаблицаЗначений;
	Для х=0 По НаборЗаписей.Fields.Count-1 Цикл
		Таблица.Колонки.Добавить(НаборЗаписей.Fields(х).Name);
	КонецЦикла;
	
	Пока Не НаборЗаписей.EOF Цикл
		СтрокаТаблицы = Таблица.Добавить();
		Для х=0 По НаборЗаписей.Fields.Count-1 Цикл
			СтрокаТаблицы[х] = НаборЗаписей.Fields(х).Value;
		КонецЦикла;
		НаборЗаписей.MoveNext();
	КонецЦикла;
	...
	Возврат XMLСтрока(Новый ХранилищеЗначения(Таблица, Новый СжатиеДанных(5)));
